...
ASR_ENABLED = os.getenv('ASR_ENABLED', os.getenv('VOICE_ENABLED', '')).lower() in ('1', 'true', 'yes')
...
# 运行模式
FULL_MODE = args.full and VOICE_ENABLED
...
def call_deepseek(messages, system_prompt=None, difficulty='中等', scene='通用销售', phase='discovery', session_context=''):
    ...
    if not (DEEPSEEK_API_KEY and DEEPSEEK_BASE_URL and DEEPSEEK_MODEL):
        return {'success': False, 'error': '模型服务未配置，请启用Mock模式或配置模型环境变量'}
    headers = {
...
⚠️ COACH和SCORE每条回复都必须有！"""
    ...
    payload = {
        ...
        "temperature": 0.8,
        "max_tokens": 400,        # 客户回复通常200字以内，400足够
        "stop": ["\n\n\n"]        # 遇到连续空行提前结束
    }
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                if response.status_code >= 500 and attempt < max_retries:
                    print(f"⚠️ {error_msg}，将重试...")
                    continue
                print(f"❌ DeepSeek API错误: {error_msg}")
            ...
            if attempt < max_retries:
                continue
            return {'success': False, 'error': f'请求超时（已重试{max_retries}次），请稍后重试'}
            
        except requests.exceptions.ConnectionError as e:
            print(f"🔌 连接失败: {e}")
            if attempt < max_retries:
                continue
            return {'success': False, 'error': f'网络连接失败（已重试{max_retries}次），请检查网络'}
...
答复: xxx【来源: xxx】"""
    ...
    try:
        ...
        return jsonify({'success': False, 'error': str(e), 'answer': ''})
...
def get_mode():
    ...
    asr_info = {
        'method': 'funasr-backend',
        'status': asr_model_status,
        'loaded': asr_model_status == 'ready',
        'error': asr_error_msg or None
    } if FULL_MODE else ({
        'method': 'web-speech-browser',
        'status': 'available',
        'loaded': True,
        'error': None
    ...
    return jsonify({
        ...
        'mock': MOCK_MODE,
        'model_configured': bool(DEEPSEEK_API_KEY and DEEPSEEK_BASE_URL and DEEPSEEK_MODEL),
    })
...
def health_check():
    ...
    return jsonify({
        ...
        'asr_error': asr_error_msg or None,
        'model_configured': bool(DEEPSEEK_API_KEY and DEEPSEEK_BASE_URL and DEEPSEEK_MODEL),
        'mock': MOCK_MODE,
        'voice_enabled': VOICE_ENABLED,
    })
...
# ===== TTS (文字转语音) 配置 =====
TTS_VOICE = os.getenv('TTS_VOICE', 'zh-CN-YunxiNeural')  # 默认男声：云希
TTS_RATE = os.getenv('TTS_RATE', '+5%')
TTS_VOLUME = os.getenv('TTS_VOLUME', '+0%')

async def _tts_generate(text, voice=None, rate=None):
    """异步生成 TTS 音频"""
    import edge_tts
...
def text_to_speech(text, voice=None, rate=None):
    """sync TTS wrapper"""
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        result = loop.run_until_complete(_tts_generate(text, voice, rate))
        loop.close()
        return result
    except Exception as e:
        print(f"[TTS] Error: {e}")
...
def tts():
    ...
    try:
        if not TTS_ENABLED:
            return jsonify({'success': False, 'error': 'TTS feature is disabled'}), 503
        data = request.json or {}
...
def list_voices():
    """获取可用语音列表"""
    voices = [
        {'id': 'zh-CN-YunxiNeural',   'name': '云希(男)',     'desc': '成熟稳重男声',       'gender': 'male'},
        {'id': 'zh-CN-YunjianNeural',  'name': '云健(男)',     'desc': '年轻阳光男声',       'gender': 'male'},
        {'id': 'zh-CN-YunyangNeural',  'name': '云扬(男)',     'desc': '新闻播报风格',       'gender': 'male'},
        {'id': 'zh-CN-XiaoxiaoNeural', 'name': '晓晓(女)',     'desc': '温柔女声',           'gender': 'female'},
        {'id': 'zh-CN-XiaoyiNeural',   'name': '晓伊(女)',     'desc': '亲和力女声',         'gender': 'female'},
        {'id': 'zh-CN-YunzeNeural',    'name': '云泽(男)',     'desc': '沉稳叙述',           'gender': 'male'},
        {'id': 'zh-CN-YunfengNeural',  'name': '云峰(男)',     'desc': '活力男声',           'gender': 'male'},
        {'id': 'zh-TW-YunjhongNeural', 'name': '云宏(男-台)',  'desc': '台湾口音男声',       'gender': 'male'},
    ]
    return jsonify({'success': True, 'voices': voices, 'default': TTS_VOICE})
