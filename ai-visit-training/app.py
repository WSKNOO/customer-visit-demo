...
def call_deepseek(messages, system_prompt=None, difficulty='中等', scene='通用销售', phase='discovery', session_context=''):
    ...
    if MOCK_MODE:
        ...
        return {'success': True, **parsed, 'usage': {}, 'mock': True}
...
def get_mode():
    ...
    return jsonify({
        ...
        'error_code': None if MOCK_MODE or MODEL_CONFIG_STATUS.configured else 'MODEL_CONFIG_INVALID',
    })
...
def health_check():
    """健康检查"""
    healthy = MOCK_MODE or MODEL_CONFIG_STATUS.configured
    payload = {
